...
class Epoch(Node):
    ...
    def __init__(self, event_trigger, before=0, after=0):
    ...
    def update(self):
        ...
        if matches is not None:
            for index, row in matches.iterrows():
                ...
                # if self._timeing_print:
                #     print("In epochs - low: ", low)
                #     print("In epochs - index: ", index)
                #     print("In epochs - high: ", high)

                if self._buffer is not None:
                    if not self._buffer.index.is_monotonic:
                        self.logger.warning(
                            "Index must be monotonic. Skipping epoch.")
                        return
        ...
        if self._epochs and self.i.ready():
            ...
            for epoch in self._epochs:
                ...
                if epoch["data"].empty:
                    ...
                    mask = (self.i.data.index >= low) & (
                        self.i.data.index <= high)
                else:
...
class ToXArray(Node):
    ...
    def update(self):
        ...
        if not self._ready:
            ports_ready = [port for _, _,
                           port in self.iterate("i*") if port.ready()]
            ...
            if port.ready():
                ...
                self._times = pd.TimedeltaIndex(
                    data=np.linspace(-self._before,
                                     self._after, self._num_times),
                    unit="s",
                )
                self._rate = 1 / \
                    (self._times[1] - self._times[0]).total_seconds()
                self._ready = True
        ...
        list_context = [port.meta["epoch"].get(
            "context") for port in ports_ready]
